...
@author: Nicolas Longuet Marx (nkd2120) 
@object: 
"""
...
class VariationalFamily(tf.keras.layers.Layer):
  ...
  def __init__(self, 
    ...
    if family in ['normal', 'lognormal', 'laplace']:
    ...
    if family == 'normal':
      ...
      self.distribution = tfp.distributions.Laplace(loc=self.location,
                                                 scale=self.scale)
      self.prior = tfp.distributions.Laplace(loc=0., scale=1.)
    else:
...
class TBIP(tf.keras.Model):
  ...
  def __init__(self,
    ...
    # nlm: laplace deviations
    self.issue_adjustment_distribution = VariationalFamily(
        'laplace',
  ...
  def get_log_prior(self,
    ...
    Args:
      ...
      issue_adjustment_samples: Samples from the ideal point variational 
        distribution. A tensor with shape [num_samples, num_authors, num_topics].
    ...
    ideal_point_log_prior = self.ideal_point_distribution.get_log_prior(
      ideal_point_samples)
     #nlm
    issue_adjustment_log_prior = self.issue_adjustment_distribution.get_log_prior(
      issue_adjustment_samples)
  ...
  def get_samples(self, seed=None):
    ...
    ideal_point_samples, seed = self.ideal_point_distribution.sample(
        self.num_samples, seed=seed)
    issue_adjustment_samples, seed = self.issue_adjustment_distribution.sample(
        self.num_samples, seed=seed)
    author_verbosity_samples, seed = self.author_verbosity_distribution.sample(
      self.num_samples, seed=seed)
    
    # let's clip the issue adjustment between -1 and 1
    issue_adjustment_samples = tf.clip_by_value(issue_adjustment_samples, -1, 1)
  ...
  def get_rate_log_prior_entropy(self,
    ...
    selected_ideal_points = tf.gather(ideal_point_samples,
                                      author_indices,
                                      axis=1)
    selected_issue_adjustment = tf.gather(issue_adjustment_samples,
                                      author_indices,
                                      axis=1)
    ...
    selected_ideological_topic_samples = tf.exp(
      (selected_ideal_points[:, :, tf.newaxis, tf.newaxis]+
       selected_issue_adjustment[:, :, :, tf.newaxis])*
  ...
  def get_ideological_term_geometric_mean(self, author_indices):
    ...
    More specifically, we compute:
      ...
      exp(E[eta_kv] * (E[x_{a_d}]+E[delta_{a_d,k}]) + E[w_{a_d}]),

    where a_d is the author of document d, delta_{a_d,k} is the author issue-adjustment on topic k 
    and w_{a_d} is the author verbosity.
    ...
    expected_ideological_term = tf.exp(
      ...
      (ideal_point_loc[:,tf.newaxis, tf.newaxis]+issue_adjustment_loc[:,:, tf.newaxis]) + 
...
def main(argv):
  ...
  for epoch in range(start_epoch + 1, FLAGS.num_epochs):
    ...
    if epoch % FLAGS.save_every == 0:
      ...
      np.save(os.path.join(param_save_dir, "issue_adjustment_loc"), 
             model.issue_adjustment_distribution.location.numpy())
      np.save(os.path.join(param_save_dir, "issue_adjustment_scale"), 
             model.issue_adjustment_distribution.scale.numpy())
