...
def build_input_pipeline(data_dir,
  ...
  variance_priors = np.load(
    os.path.join(data_dir, "variance_priors.npy"))
  prior_eta_location= np.load(
    os.path.join(data_dir, "prior_eta_location.npy"))
  prior_beta_location= np.load(
    os.path.join(data_dir, "prior_beta_location.npy"))
  author_map = np.loadtxt(os.path.join(data_dir, "author_map.txt"),
                          dtype=str,
                          delimiter="\n")
  #nlm: could add encoding
  ...
  vocabulary = np.loadtxt(os.path.join(data_dir, "vocabulary.txt"),
                          ...
                          delimiter="\n",
                          comments="<!-")
  return dataset, vocabulary, author_map,  num_documents, num_words, initial_ideal_points, variance_priors, prior_eta_location, prior_beta_location
...
class VariationalFamily(tf.keras.layers.Layer):
  ...
  def __init__(self, 
               ...
               prior_eta_location=None,
               prior_beta_location=None):
    ...
    if family == 'normal':
      ...
      if initial_ideal_points is not None:
        if variance_priors is not None:
            self.prior = tfp.distributions.Normal(loc=initial_ideal_points.astype(np.float32), scale=variance_priors.astype(np.float32))
        else: 
      ...
      elif prior_eta_location is not None:
           self.prior = tfp.distributions.Normal(loc=prior_eta_location.astype(np.float32), scale=np.ones(prior_eta_location.shape).astype(np.float32))
      else: 
        self.prior = tfp.distributions.Normal(loc=0., scale=1.)
    elif family == 'lognormal':
      self.distribution = tfp.distributions.LogNormal(loc=self.location,
                                                      scale=self.scale)
      if prior_beta_location is not None: 
        self.prior = tfp.distributions.Gamma(concentration=prior_beta_location.astype(np.float32), rate=np.ones(prior_beta_location.shape).astype(np.float32)*0.3)
      else: 
...
class TBIP(tf.keras.Model):
  ...
  def __init__(self,
    ...
    Args:
      ...
      variance_priors: A numpy array (S x J) containing the variance 
        of the priors
      prior_eta_location: A numpy array (V x J) containing the priors on the
      the location of the ideological topic 
      prior_beta_location: A numpy array (V x J) containing the priors on the
      the location of the neutral topic  
    ...
    self.objective_topic_distribution = VariationalFamily(
      ...
      cavi=cavi,
      # nlm: not sure exaxctly what this is going to do 
      prior_beta_location = prior_beta_location)
    self.ideological_topic_distribution = VariationalFamily(
      'normal',
      [num_topics, num_words],
      prior_eta_location = prior_eta_location)
    ...
    self.ideal_point_distribution = VariationalFamily(
      ...
      variance_priors=variance_priors)
  ...
  def get_ideological_term_geometric_mean(self, author_indices):
    ...
    expected_ideological_term = tf.exp(
      ...
      #nlm: here no need for tf.newaxis for second axis. 
...
def main(argv):
  ...
  (dataset, vocabulary, author_map,
   num_documents, num_words, initial_ideal_points, variance_priors, prior_eta_location, prior_beta_location) = build_input_pipeline(
      data_dir,
      FLAGS.batch_size,
      random_state,
