...
def build_input_pipeline(data_dir,
  ...
  author_indices = np.load(
    os.path.join(data_dir, "author_indices.npy")).astype(np.int64)
  initial_ideal_points = np.load(
    os.path.join(data_dir, "initial_ideal_points.npy")).astype(np.int64)
  ...
  def __init__(self, 
    ...
    if family == 'normal':
      self.distribution = tfp.distributions.Normal(loc=self.location,
                                                   scale=self.scale)
      if initial_ideal_points is not None:
        self.prior = tfp.distributions.Normal(loc=initial_ideal_points.astype(np.float64), scale=np.ones(initial_ideal_points.shape).astype(np.float64)/10**8)
      else: 
  ...
  def __init__(self,
    ...
    Args:
      ...
      num_samples: The number of Monte-Carlo samples to use to approximate the
        ELBO.
      initial_ideal_points: A numpy array (S x J) containing the initial ideal points for politicians 
